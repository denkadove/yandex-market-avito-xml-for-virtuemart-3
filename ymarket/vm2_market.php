...
$xml .= '<enable_auto_discounts>yes</enable_auto_discounts>'."\n";
$xml .= '<currencies>'."\n";
$xml .= '<currency id="'.CURRENCY.'" rate="1"/>';
$xml .= '</currencies>'."\n";
//$xml .= getProductId();
$xml .= '<categories>'."\n";
$query = 'SELECT a.category_parent_id, a.category_child_id, b.category_name FROM #__virtuemart_category_categories a RIGHT JOIN #__virtuemart_categories_'.$lang.' b ON b.virtuemart_category_id = a.category_child_id WHERE a.category_child_id IN ('.EXCLUDE_CAT.') ORDER BY a.category_child_id';
$db->setQuery($query);
...
foreach ($rows as $row) {
	...
	//$xml .= '<weight>'.$product_weight.'</weight>'."\n";
	if ($row->mf_name) {
	...
	if ($row->product_desc) {
		$xml .= '<description>'.htmlspecialchars(mb_substr(strip_tags($row->product_desc), 0, 2950)).'</description>'."\n";
